    ...
    <div className="flex flex-col flex-1 bg-background relative overflow-hidden min-h-[calc(100vh-64px)]">
      ...
      <div className="pt-8 px-6 pb-20 relative z-10 max-w-2xl mx-auto w-full">
        <div className="flex items-center justify-between mb-8">
          ...
          <h1 className="font-semibold text-xl text-textPrimary">{t('profile.language')}</h1>
          <div className="w-11 h-11" />
        </div>
        <div className="premium-card p-3 flex flex-col">
          {LANGUAGES.map((lang, index) => (
            <React.Fragment key={lang.code}>
              <button onClick={() => changeLanguage(lang.code)} className={`flex items-center justify-between p-4 w-full text-left rounded-2xl transition-colors ${i18n.language === lang.code ? 'bg-avatarBg' : 'hover:bg-surfaceMuted'}`}>
                <div className="flex items-center">
                  <div className="w-8 flex justify-center mr-3 shrink-0"><img src={`https://flagcdn.com/w40/${lang.isoCode}.png`} alt={lang.label} className="w-5 h-auto rounded-[2px] shadow-sm" /></div>
                  <div>
                    <span className={`block font-semibold text-base ${i18n.language === lang.code ? 'text-primary' : 'text-textPrimary'}`}>{lang.nativeLabel}</span>
                    <span className="block font-medium text-sm text-textMuted mt-0.5">{lang.label}</span>
                  </div>
                </div>
                {i18n.language === lang.code && <Check size={20} className="text-primary" />}
              </button>
              {index < LANGUAGES.length - 1 && <div className="h-px bg-border w-full my-1" />}
            </React.Fragment>
          ))}
        </div>
      </div>
